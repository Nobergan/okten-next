...
const NewCarPage: React.FC = () => {
  ...
  const errorInput = "border-red-400 focus:border-red-400 focus:ring-red-500/30";
  const labelCls = "mb-1.5 block text-sm font-medium text-slate-200";
  const helpErrCls = "mt-1 text-sm text-red-300";
  const cardCls = "rounded-2xl border border-white/10 bg-white/10 backdrop-blur p-6 shadow-xl";
  const btnBase =
    "inline-flex w-full items-center justify-center gap-2 rounded-2xl px-5 py-3 font-semibold " +
    "text-white shadow-lg shadow-emerald-600/25 transition focus:outline-none disabled:opacity-60 " +
    "disabled:cursor-not-allowed bg-emerald-600 hover:bg-emerald-500";
  ...
  return (
    <section className="min-h-screen flex items-center justify-center px-6 bg-gradient-to-b from-slate-900 via-slate-950 to-black">
      <div className="w-full max-w-xl text-center">
        ...
        <p className="mt-3 text-slate-300">Fill the fields below to add a new car.</p>
        ...
        <div className={`mt-8 text-left ${cardCls}`}>
          ...
          >
            ...
            <button type="submit" className={btnBase} disabled={submitting}>
              {submitting && (
